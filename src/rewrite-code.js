...
const { ownkeys, isMainChunk } = require("./util");
...
module.exports = function rewriteCode(sources) {
  const { chunks, mapModuleNameToId, modules: allModules } = sources;
  for (let i = 0; i < chunks.length; i++) {
    const { modules } = chunks[i];
    for (let j = 0; j < modules.length; j++) {
      ...
      traverse(ast, {
        CallExpression(path) {
          if (path.node.callee.name === "require") {
            const absoulutePath =
              mapModuleNameToId[path.node.arguments[0].value];
            path.replaceWith(
        ...
        ImportDeclaration(nodePath) {
          ...
          const importSource = allModules[absoulutePath];
        ...
        Import(path) {
          ...
          const asyncModule = allModules[absoulutePath];
          // 动态导入的模块应该只有一个chunk
          const chunk = asyncModule.chunks[0];
          let expression;
          if (isMainChunk(chunk)) {
            ...
            expression = t.callExpression(
              ...
              [t.numericLiteral(asyncModule.chunks[0].id)]
            );
          }
            ...
            t.callExpression(
              ...
              [
                t.callExpression(
                  ...
                  [t.nullLiteral(), t.stringLiteral(absoulutePath)]
                ),
              ]
            )
          );
      ...
      traverse(ast, {
        Program(path) {
          ...
          // 修改导入变量名
          const keys = ownkeys(identifierMap);
